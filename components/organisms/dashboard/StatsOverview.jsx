...
const stats = [
    ...
    { label: "Total Uptime", value: "3643hrs" },
];
...
export default function StatsOverview() {
    ...
    // });
    return (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-nowrap">
            {stats.map((stat, i) => (
                <div key={i} className="rounded-xl p-2 sm:p-4 space-y-4 text-center shadow-sm bg-gradient-to-br from-green-50 via-white to-green-100/80 backdrop-blur-xl">
                    ...
                    <h3 className="text-2xl font-bold flex justify-start text-accent">{stat.value}</h3>

                    {/* <h3 className="text-2xl font-bold flex justify-start">{inView && <CountUp end={stats.value} duration={2} />}</h3> */}
                </div>
            ))}
        </div>
    )
