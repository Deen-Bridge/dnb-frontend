 ...
 * @returns {Promise<Object>} - { timings, hijriDate, gregorianDate, locationName }
 */
export async function fetchPrayerTimes(location) {
  ...
  if (location?.lat && location?.lng) {
    ...
    // Default fallback city (Mecca / Cairo / London)
    apiUrl = `https://api.aladhan.com/v1/timingsByCity?city=Mecca&country=Saudi Arabia&method=2`;
 ...
 */
export function getNextPrayerInfo(timings) {
  ...
  for (const name of prayerNames) {
    ...
    if (prayerDate > now) {
      const remainingSeconds = Math.max(0, Math.floor((prayerDate.getTime() - now.getTime()) / 1000));
      return {
        nextPrayerName: name,
  ...
  const remainingSeconds = Math.max(0, Math.floor((tomorrowFajr.getTime() - now.getTime()) / 1000));
