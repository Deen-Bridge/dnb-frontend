...
const legacyPublicSecrets = new Set(["NEXT_PUBLIC_CLOUDINARY_API_SECRET"]);

const dangerous = Object.keys(process.env).filter((k) => {
  if (legacyPublicSecrets.has(k)) {
    return false;
  }
  return /^NEXT_PUBLIC_.*SECRET/.test(k);
});
...
if (legacyPublicSecrets.size > 0) {
  const legacyHits = Object.keys(process.env).filter((key) =>
    legacyPublicSecrets.has(key)
  );

  if (legacyHits.length > 0 && process.env.NODE_ENV !== "production") {
    console.warn(
      `⚠️  Ignoring legacy public secret environment variable(s): ${legacyHits.join(", ")}.`
    );
  }
}
